...
class TestClassModel:
    ...
    def test_features_relationship(self):
        ...
        klass = ClassFactory(name=ClassName.FIGHTER)
        # Use unique names to avoid conflict with fixture data
        feature1 = ClassFeatureFactory(klass=klass, name="Test Feature One", level=1)
        feature2 = ClassFeatureFactory(klass=klass, name="Test Feature Two", level=1)

        # Fixtures load 3 features for Fighter, plus 2 we added = 5
        assert klass.features.count() >= 2
        assert feature1 in klass.features.all()
        assert feature2 in klass.features.all()

    def test_ordering(self):
        """Classes are ordered by name alphabetically."""
        # Fixtures load all 12 classes, ordered alphabetically
        classes = list(Class.objects.all())
        # First class alphabetically is 'barbarian'
        assert classes[0].name == ClassName.BARBARIAN
...
class TestClassFeatureModel:
    ...
    def test_creation(self, fighter):
        # Use unique test name to avoid fixture conflicts
        feature = ClassFeature.objects.create(
            name="Test Creation Feature",
            klass=fighter,
            level=1,
            description="Test feature description.",
        )
        assert feature.name == "Test Creation Feature"
        assert feature.klass == fighter
        assert feature.level == 1
        assert "Test feature" in feature.description

    def test_str(self, fighter):
        feature = ClassFeature.objects.create(
            name="Test Str Feature",
            klass=fighter,
            level=1,
            description="Test feature.",
        )
        assert str(feature) == "Test Str Feature (Level 1)"

    def test_str_higher_level(self, fighter):
        feature = ClassFeature.objects.create(
            name="Test Higher Level Feature",
            klass=fighter,
            level=2,
            description="Take an additional action.",
        )
        assert str(feature) == "Test Higher Level Feature (Level 2)"
    ...
    def test_unique_together_constraint(self, fighter):
        ...
        ClassFeature.objects.create(
            name="Test Unique Feature", klass=fighter, level=1, description="Test"
        )
        with pytest.raises(Exception):
            ClassFeature.objects.create(
                name="Test Unique Feature",
                klass=fighter,
                level=4,
                description="Another",
            )
    ...
    def test_ordering(self, fighter):
        ...
        # Filter to just fighter features to check ordering
        ClassFeature.objects.create(
            name="ZZZ Test Feature", klass=fighter, level=2, description="Test"
        )
        ClassFeature.objects.create(
            name="AAA Test Feature", klass=fighter, level=1, description="Test"
        )
        ClassFeature.objects.create(
            name="BBB Test Feature", klass=fighter, level=1, description="Test"
        )

        features = list(
            ClassFeature.objects.filter(klass=fighter).order_by("level", "name")
        )
        # First level 1 features alphabetically
        level_1_features = [f for f in features if f.level == 1]
        assert (
            level_1_features[0].name < level_1_features[-1].name
        )  # Alphabetically sorted

    def test_cascade_delete_on_class(self, fighter):
        """ClassFeature should be deleted when class is deleted."""
        ClassFeature.objects.create(
            name="Test Cascade Feature", klass=fighter, level=1, description="Test"
        )
